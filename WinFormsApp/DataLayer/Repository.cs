...
namespace DBStudentApp_1216098.DataLayer
{
    internal class Repository : IRepository
    {
        ...
        public List<Enrollment> GetEnrollment(string courseNum)
        {
            ...
            try
            {
                string sql = "select s.StudentId,s.FirstName,s.LastName,c.Credits " + "from Students s " + "join enrollments e on s.StudentId=e.StudentId " + "join courses c on e.coursenum=c.coursenum " + "where c.CourseNum='" + courseNum + "'";
                DataTable dt = _idac.GetManyRowsCols(sql);
